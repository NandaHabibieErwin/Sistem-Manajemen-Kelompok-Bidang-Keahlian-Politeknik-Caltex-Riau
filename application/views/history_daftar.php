...
<head>
	<meta charset="utf-8">
	<title>Riwayat Pendaftaran - KBK PCR</title>
...
<body>
	...
	<div id="layoutSidenav_content">
		<main>
			<div class="container-fluid px-4">
				<h1 class="mt-4">Tabel Pendaftaran</h1>
				<ol class="breadcrumb mb-4">

					<li class="breadcrumb-item active"><a href="<?php echo site_url('Users/history'); ?>">Tabel
							Dosen</a></li>
					<li class="breadcrumb-item active"><a href="<?php echo site_url('KBK/history'); ?>">Tabel
							KBK</a></li>
					<li class="breadcrumb-item">Tabel Pendaftaran KBK</li>
					<li class="breadcrumb-item active"><a href="<?php echo site_url('Kegiatan/history'); ?>">Tabel
							Event KBK</a></li>
				...
				<div class="card mb-4">
					<div class="card-body">
						<table id="datatablesSimple">
							...
							<tbody>
								<?php
								$data['kegiatan'] = $this->daftarModel->getAllDaftar();
								foreach ($data['kegiatan'] as $row): ?>
									<tr>
										<td>
											<?php echo $row['name']; ?>
										</td>
										<td>
											<?php echo $row['nama_kbk_id_kbk']; ?>
										</td>
										<td>
											<?php echo $row['nama_kbk_id_kbkp']; ?>
										</td>
										<td>
											<?php echo $row['matkul']; ?>
										</td>
										...
										<td>
											<?php if ($row['status'] !== 'Disetujui' && $row['status'] !== 'Ditolak'): ?>
												<button class="btn-primary" onclick="assignKbkKbkp('<?php echo $row['id_daftar']; ?>', 
							'<?php echo $row['id_kbk']; ?>', 
							'<?php echo $row['id_kbkp']; ?>')">Assign KBK</button>
												<button class="btn-primary" onclick="tolakDaftar('<?php echo $row['id_daftar']; ?>', 
							'<?php echo $row['id_kbk']; ?>')">Tolak</button>
											<?php endif; ?>
										</td>
									</tr>
								<?php endforeach; ?>
	...
	<script>
		function tolakDaftar(id_daftar) {
			// Make an AJAX request to the controller method with the data

			$.ajax({
				url: "<?php echo base_url('Daftar/tolakDaftar'); ?>",
				type: "POST",
				data: {
					id_daftar: id_daftar
				},
				...
				success: function (response) {
					...
					// Clear the DataTables cache and reload the table
